...
# I had problem running the test_main.py code so i used chatgpt to fix the proble
...
# def test_save_enteries(t_path):
#     test_file = tmp_path / "test_appData.csv"
#     test_file.write_text("Date,Category,Amount\n")
#     app = FinanceApp(csv_file))
...
# def test_load_file():
#     test_file = Path("test_appData.csv")
#     if test_file.exists():
#         contents = test_file.read_text()
#         assert "Bonus" in contents
#         assert "100" in contents
#     new_app = FinanceApp(csv_file=str(test_file))
#     assert new_app.balance == 120
#     assert len(new_app.record_history) == 1
#     assert new_app.record_history[0][1] == "Bonus"
#

# ChatGPT was used to fix some of the problems in this section of the code
...
def test_initial_balance_and_history():
    ...
    # This function was assisted by ChatGPT to optimize performance:
    temp_file.unlink()
    # temp_file.unlink(),  to convert the csv file to string (=str(temp_file)
